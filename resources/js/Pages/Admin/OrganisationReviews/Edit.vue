...
const update = () => {
  const url = route('admin.organisation-reviews.update', props.companyReview.id)
  form.put(url, {
    onSuccess: () => {
...
<template>
  <main class="container flex-grow p-4 sm:p-6">
    <HeaderSegment :title="trans('Edit Organisation Review')" :segments="segments" :buttons="buttons" />
    <div class="space-y-6">
      <form @submit.prevent="update" class="ajaxform_instant_reload" enctype="multipart/form-data">
        <div class="grid lg:grid-cols-5">
          <div class="lg:col-span-2">
            <strong>{{ trans('Edit organisation reviews') }}</strong>
