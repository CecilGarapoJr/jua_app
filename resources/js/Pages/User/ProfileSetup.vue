...
const validateDegree = (index) => {
  // Clear the error for this field if a value is selected
  if (form.education_qualifications[index].degree) {
    delete form.errors['education_qualifications.' + index + '.degree']
  }
}
...
<template>
  ...
  <div class="dashboard-body container mx-auto mt-4 rounded p-5">
    <h2 class="main-title ml-5">{{ trans('Setup your profile') }}</h2>
    ...
    <form @submit.prevent="nextStep">
      ...
      <div v-if="step == 3">
        ...
        <!-- work experiences -->
        <div class="mt-30">
          <p class="fw-bold">{{ trans('Experience') }}</p>
          <div
        ...
        <button type="button" @click="addWorkExperienceItem" class="btn-two">
          <i class="bi bi-plus"></i> {{ trans('Add Experience') }}
        </button>
      </div>
              ...
              <div class="row">
                <div class="col-md-2 dash-input-wrapper mb-2">
                  ...
                  <NiceSelect
                    ...
                    @change="validateDegree(index)"
                  />
                  
                  <div v-if="!item.degree && form.errors['education_qualifications.' + index + '.degree']" class="text-danger mt-1">
                    <small>{{ trans('The degree field is required') }}</small>
                  </div>
