...
// Redirect installer routes to home
Route::any('install/{any?}', function() { return redirect('/'); })->where('any', '.*');

// Include terminology routes (new URL patterns)
require __DIR__ . '/terminology-routes.php';

// Test routes for terminology changes
Route::get('/test-terminology', function() {
    return view('test-terminology');
})->name('test-terminology');

// Terminology audit route
Route::get('/terminology-audit', [App\Http\Controllers\TerminologyAuditController::class, 'index'])->name('terminology-audit');

Route::get('/test-terminology-frontend', function() {
    return inertia('TerminologyFrontendTest');
})->name('test-terminology-frontend');
